...
/**
 * ProfileCard — 3D holographic identity card with tilt, glow, and shine effects.
 *
 * Ported from ReactBits (https://reactbits.dev/components/profile-card).
 * CSS lives in `_profile-card.scss`.
 *
 * @see react_bits/ProfileCard.tsx for the original source.
 */
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const handleDeviceOrientation = useCallback((e: DeviceOrientationEvent) => {
    ...
    const cx = s.clientWidth / 2; const cy = s.clientHeight / 2;
    tiltEngine.setTarget(
      clamp(cx + gamma * mobileTiltSensitivity, 0, s.clientWidth),
      clamp(cy + (beta - ANIMATION_CONFIG.DEVICE_BETA_OFFSET) * mobileTiltSensitivity, 0, s.clientHeight)
    );
  ...
  useEffect(() => {
    ...
    const pMove = handlePointerMove as EventListener;
    const pEnter = handlePointerEnter as EventListener;
    const pLeave = handlePointerLeave as EventListener;
    const dOrientation = handleDeviceOrientation as EventListener;

    shell.addEventListener('pointerenter', pEnter);
    shell.addEventListener('pointermove', pMove);
    shell.addEventListener('pointerleave', pLeave);

    const handleClick = () => {
      if (!enableMobileTilt || location.protocol !== 'https:') return;
      const dm = window.DeviceMotionEvent as unknown as { requestPermission?: () => Promise<string> };
      if (dm && typeof dm.requestPermission === 'function') {
        dm.requestPermission().then(s => { if (s === 'granted') window.addEventListener('deviceorientation', dOrientation); }).catch(console.error);
      } else { window.addEventListener('deviceorientation', dOrientation); }
    };
    ...
    return () => {
      shell.removeEventListener('pointerenter', pEnter);
      shell.removeEventListener('pointermove', pMove);
      shell.removeEventListener('pointerleave', pLeave);
      shell.removeEventListener('click', handleClick);
      window.removeEventListener('deviceorientation', dOrientation);
      if (enterTimerRef.current) window.clearTimeout(enterTimerRef.current);
      if (leaveRafRef.current) cancelAnimationFrame(leaveRafRef.current);
      tiltEngine.cancel();
